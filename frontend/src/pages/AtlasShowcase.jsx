...
const ASSETS = {
  ...
  youtubeUrl: "https://www.youtube.com/watch?v=M5wxTALfgXM",
...
// single-line “share” pill used in the hero
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: "Atlas AI — AI built for your business.",
          text: "Check out Atlas AI — an operational AI copilot for supply chain and everyday businesses.",
          url: shareUrl,
        });
        setStatus("Shared!");
    ...
    <div className="w-auto inline-block">
      <button
        ...
        className={`inline-flex items-center gap-1 rounded-full px-3 py-[3px] text-xs font-medium text-slate-100 
          ...
          ${status ? "animate-pulse ring-2 ring-cyan-400/60" : ""}`}
        aria-label="Share Atlas"
        title={status || "Share this page"}
                      ...
                      {open && (
                        <tr className="border-b border-slate-200 bg-slate-50/40">
                          <td colSpan={4} className="pl-4 pr-4 py-4">
                            <div
                              className="border border-slate-200 rounded-xl overflow-x-auto overflow-y-auto max-h-[260px] bg-white shadow-inner text-[12px] font-mono text-slate-900"
                            ...
                            <div className="text-[11px] text-slate-500 mt-2 leading-snug">
                              This is real ERP data. Filter it live, copy any
                              ID, and then paste into Atlas on the right.
                            </div>
                          </td>
                        </tr>
                      )}
...
export default function AtlasShowcase() {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([entry]) =>
        setChatInView(entry.isIntersecting && entry.intersectionRatio > 0.2),
      { threshold: [0.2] }
    );
    io.observe(node);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      {/* Top nav pinned */}
      <SectionNav onOpenChat={() => setChatOpen(true)} hideNav={chatOpen} />
      ...
      <div className="relative z-10">
              ...
              <div className="flex flex-col justify-between rounded-2xl p-6 md:p-8 w-full bg-gradient-to-r from-slate-900 to-slate-800 text-white shadow-lg ring-1 ring-white/10 min-h-[480px]">
                <div>
                  ...
                  <p className="mt-3 text-sm text-slate-400 max-w-xl leading-relaxed">
                    Born in supply chain. Built to scale across industries.
                  </p>
                </div>
        ...
        <main className="mx-auto max-w-6xl px-4 pb-24 space-y-20 md:space-y-24">
          ...
          {/* SEE ATLAS IN ACTION – card */}
          <section id="see-atlas-in-action">
            <div className="rounded-2xl bg-white/85 backdrop-blur shadow-xl ring-1 ring-slate-200/70 p-6 md:p-8">
              <SeeAtlasInAction />
            </div>
          </section>

          {/* SUPPLY CHAIN – card with light blue tint */}
          <section id="supply-chain">
            <div className="rounded-2xl bg-blue-50/70 backdrop-blur shadow-xl ring-1 ring-blue-100/70 p-6 md:p-8">
              <SupplyChainDeepDive />
            </div>
          </section>

          {/* EVERYDAY BUSINESSES – card */}
          <section id="industries">
            <div className="rounded-2xl bg-white/90 backdrop-blur shadow-xl ring-1 ring-slate-200/70 p-6 md:p-8">
              <IndustriesPreview />
            </div>
          </section>

          {/* BEHIND ATLAS – card */}
          <section id="behind-atlas">
            <div className="rounded-2xl bg-white/90 backdrop-blur shadow-lg ring-1 ring-slate-200/70 p-4 md:p-6">
              <BehindAtlas />
            </div>
          </section>

          {/* FINAL CTA – slightly stronger tint so it feels like a footer card */}
          <section id="final-cta" className="mt-24 py-16">
        ...
        <div className="fixed right-3 bottom-3 sm:right-6 sm:bottom-6 z-50">
          ...
          >
            ...
            <span className="sm:hidden">Try Atlas Now</span>
          </button>
        </div>

        {/* Floating Chat Modal */}
        {chatOpen && (
          <ModalOverlayWithRails onClose={() => setChatOpen(false)} />
        )}
